...
from spots.models import TextileRecyclingTrash
import requests
import logging
...
class Command(BaseCommand):
    help = 'Imports TextileRecyclingTrash data from an external API'

    def handle(self, *args, **options):
        self.fetch_and_import_records()

    def fetch_and_import_records(self):
        base_url = "https://opendata.paris.fr/api/explore/v2.1/catalog/datasets/dechets-menagers-points-dapport-volontaire-conteneur-textile/records"
        offset = 0
        limit = 100
        ...
        while True:
            ...
            for record in results:
                print(record)
                try:
                    formatted_lon = record['longitude']
                    formatted_lat = record['latitude']
                    unique_id = f"{formatted_lon}_{formatted_lat}"

                    # Utilisation de update_or_create pour insérer ou mettre à jour une textile_trash
                    textile_trash, created = TextileRecyclingTrash.objects.update_or_create(
                        trash_id=unique_id,
                        defaults={
                            'address': record['adresse'],
                            'district': record['code_postal'],
                            'operator': record['operateur'],
